...
Then open **<http://127.0.0.1:7869>**.

Compose binds to `127.0.0.1` by default. To expose the container to your LAN, change the port mapping to `${PORT:-7869}:7869`, set `BASIC_AUTH_USER`, `BASIC_AUTH_PASS`, and `ADMIN_SECRET`, then restart. See [Access Control](../CONFIGURATION.md#access-control).
...
docker run -d -p 127.0.0.1:7869:7869 -v marinara-data:/app/data marinara-engine
```
...
```bash
podman run -d -p 127.0.0.1:7869:7869 -v marinara-data:/app/data ghcr.io/pasta-devs/marinara-engine:latest
```
...
docker run -d -p 127.0.0.1:7869:7869 -v marinara-data:/app/data ghcr.io/pasta-devs/marinara-engine:lite
```

Or with Podman:

```bash
podman run -d -p 127.0.0.1:7869:7869 -v marinara-data:/app/data ghcr.io/pasta-devs/marinara-engine:lite
```
...
docker run -d -p 127.0.0.1:7869:7869 -v marinara-data:/app/data marinara-engine:lite
```
